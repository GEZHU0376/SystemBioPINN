...
class Brain:
    ...
    def Init(cls, data, net, criterion, optimizer, lr, iterations, lbfgs_steps, path = None, batch_size=None, 
             batch_size_test=None, print_every=1000, save=False, callback=None, dtype='float', device='cpu'):
        ...
        for i in range(self.iterations + 1):
            ...
            if i % self.print_every == 0 or i == self.iterations:
                ...
                if self.callback is not None: 
                    MSE = self.callback(self.data, self.net)
                    loss_history.append([i, loss.item(), loss_test.item()]+MSE)   
                else:
